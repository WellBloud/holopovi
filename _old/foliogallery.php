...
$itemsPerPage        = '15';    // number of images per page if not already specified in ajax mode
$thumb_width         = '200';   // width of thumbnails in pixels
$sort_albums_by_date = FALSE;   // TRUE will sort albums by upload date, FALSE will sort albums by name
$sort_images_by_date = FALSE;   // TRUE will sort thumbs by creation date, FALSE will sort images by name
...
{		
	...
	{ ?>
		...
		<div class="titlebar"><p>Žádná alba nejsou vytvořena.</p></div>
	...
	else
	{
		...
		<div class="p10-lr">
            ...
            if($numAlbums == 1) {
                $nazev = 'složka';
            } else if($numAlbums > 1 && $numAlbums < 5) {
                $nazev = 'složky';
            } else {
                $nazev = 'složek';
            }
            ?>
        	<span class="title"><?php echo $numAlbums . ' ' . $nazev; ?> v galerii</span>
        </div>
...
else //display photos in album 
{
	...
	<div class="p10-lr">
		<?php if($fullAlbum==1) { ?>
			<span class="title"><a href="<?php echo $album_page_url; ?>" class="refresh">Seznam složek</a></span>
			<span class="title">&raquo;</span>
		<?php } ?>
		<span class="title"><?php echo $_REQUEST['album']; ?></span> - <?php echo $numFiles; ?> obrázků
	</div>  
	...
	{ ?>
		 ...
		 <div class="p10-lr"><p>V albu nejsou žádné obrázky.</p></div>
